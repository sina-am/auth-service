...
class AuthenticationService:
    ...
    def authenticate(self, credentials: Union[RealUserAuthenticationIn, LegalUserAuthenticationIn]) -> Union[RealUser, LegalUser]:
        try:
            if isinstance(credentials, RealUserAuthenticationIn):
                user = self.database.users.get_by_national_code(credentials.national_code)
            else:
                user = self.database.users.get_by_company_code(credentials.company_code)
    ...
    def create_admin(self, national_code, phone_number, first_name, last_name, password) -> RealUser:
        try:
            ...
            self.database.roles.create(Role(platform='*', names=['admin'])) # type: ignore
