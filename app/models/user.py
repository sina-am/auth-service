...
class ProfileOut(BaseModelOut):
    ...
    def from_db(cls,
        ...
        return cls(
            ...
            contact_information=ContactInformationOut.from_db(
...
class UserUpdateIn(BaseModelIn):
    ...
    def to_model(self, user: Union[RealUser, LegalUser]) -> Union[RealUser, LegalUser]:
        ...
        if user.type == UserType.REAL and self.real_user:
            ...
            user.birth_day = date_to_datetime(
                self.real_user.birth_day) if self.real_user.birth_day else user.birth_day
...
class PasswordUpdateIn(BaseModel):
    verification: Union[RealUserCodeVerificationIn,
                        LegalUserCodeVerificationIn]
    password1: str
    password2: str
...
class PhoneNumberUpdateIn(BaseModel):
    ...
    def check_if_match(cls, v: PhoneNumberField, values):
        if values.get('verification') and v == values['verification'].phone_number:
            return v
        raise ValueError(
            _("phone_number should be eqaul to the verification.phone_number"))
